      ...
      <div className="p-4 rounded-xl border border-slate-200 bg-white shadow-2xs flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <Button
            ...
            disabled={isScanning}
            onClick={handleScan}
            className="text-slate-700 hover:text-brand-800 text-sm flex items-center gap-1.5"
          >
          ...
          <Button
            ...
            onClick={handleReset}
            className="text-rose-700 border-rose-200 hover:bg-rose-50 text-sm flex items-center gap-1.5"
          >
          ...
          <div className="divide-y divide-slate-100">
            {report.issues.map((issue, idx) => (
              <div key={idx} className="p-4 space-y-1.5 hover:bg-slate-50/50 transition-colors">
                ...
                <p className="text-slate-600 text-xs leading-relaxed">{issue.issue}</p>

                <div className="p-2 rounded bg-slate-50 border border-slate-200/60 text-xs text-slate-700 flex items-start gap-1.5">
